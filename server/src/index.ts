...
// Ensure DB is ready before processing any request
app.use(async (req, res, next) => {
    try {
        ...
        res.status(503).json({ 
            error: "Database is not connected. Check Atlas Network Access (whitelist your IP) and the MONGODB_URI credentials."
        });
    }
});
